...
[**zapret-kur.bat**](https://github.com/ahmetenesdur/zapret-turkiye/blob/main/zapret-kur.bat) dosyasına tıkla, açılan sayfadaki indirme butonunu kullanarak indir.
...
```
=============================================
  ZAPRET TURKIYE KURULUM ARACI  v1.0
=============================================
...
7 adımı otomatik çalıştırır. İnternet bağlantısı gereklidir.

Strateji testi sırasında tüm stratejiler sırayla denenir. İlk başarılı strateji seçilir ve servis olarak kurulur. Bu adım ISP'ye bağlı olarak **2–5 dakika** sürebilir.
...
- `C:\zapret` klasörünü temizler
- Defender dışlamasını kaldırır
- İstersen DNS ayarını da sıfırlar

### Güncelle (menü 3)

Mevcut stratejiyi ve son sürümü gösterir. Onaylarsan eski kurulumu temizleyip yeni sürümü kurar. DNS ayarına dokunmaz.
...
| Discord erişimi | Gerçek zamanlı bağlantı testi                    |
...
Metin kanalları açılıyor ama sesli kanala bağlanamıyorsan, bazı Discord ses sunucuları IP seviyesinde engelli olabilir. Çözümü:
...
3. **İndirme** — GitHub Releases API → `Invoke-WebRequest`
4. **Çıkarma** — `Expand-Archive` + `Unblock-File` + nested folder düzeltme
5. **Defender** — `Add-MpPreference -ExclusionPath`
6. **Çakışma** — 8 bilinen servis için `Get-Service` → `sc.exe delete`
7. **Strateji testi** — Her strateji BAT'ını `Start-Process -WindowStyle Hidden` ile çalıştırır, `discord.com/api/v10/gateway` endpoint'ine GET isteği ile test eder
8. **Servis** — BAT dosyasından `winws.exe` argümanlarını parse eder → `New-Service` ile Windows servisi oluşturur
