...
| 6   | **Strateji testi**    | 3 fazlı test: Türkiye stratejileri → parametre taraması → genişletilmiş tarama |
...
```
=============================================
  ZAPRET TURKIYE KURULUM ARACI  v1.2
=============================================
  [ DURUM: KALKAN AKTIF ]
  Strateji: general
...
Strateji testi **3 fazlı** çalışır:

1. **Faz 1** — Türkiye ISP presetleri (Türk Telekom, Superonline, Kablonet, Turkcell, Vodafone) + genel stratejiler + bundle stratejileri sırayla denenir (~1-3 dk)
2. **Faz 2** — Faz 1 başarısız olursa, kullanıcı onayıyla 40 parametre kombinasyonu otomatik taranır (~5-10 dk)
3. **Faz 3** — Faz 2 de başarısız olursa, 360 genişletilmiş kombinasyon denenebilir (~10-20 dk)

İlk başarılı strateji seçilir, servis olarak kurulur ve Discord erişimi doğrulanır. Çoğu kullanıcı Faz 1'de **1 dakika içinde** çözüm bulur.
...
8. **Strateji testi** — 3 fazlı: önce ISP presetleri + genel Türkiye stratejileri, sonra parametre sweep, son olarak genişletilmiş sweep. Her faz `discord.com/api/v10/gateway` ile doğrulanır
9. **Servis** — Kazanan strateji doğrudan parametrelerle (Turkey/sweep) veya BAT parse ile → `New-Service` ile Windows servisi oluşturur → upstream sürümünü Registry'ye kaydeder
